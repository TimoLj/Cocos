...
export class KBEngineApp
{
    ...
    getServerErrorsName(code){
        return this.serverErrors[code].name
    }

    getServerErrorsDesc(code){
        return this.serverErrors[code].description
    }
	...
	Client_onCreateAccountResult(stream: MemoryStream)
	{
		...
		if(retcode != 0)
		{
            KBEEvent.Fire("onCreateAccountResult", this.serverErrors[retcode].name, datas);
		...
		KBEEvent.Fire("onCreateAccountResult", "", datas);
